...
int main(int argc, char *argv[]){
    string filename;
    int kval,wait,nopivotPSize,degeneracySplit,minvade;
    char splitType;

    filename=argv[1];
    kval=stoi(argv[2]);
    nopivotPSize=stoi(argv[3]);
    degeneracySplit=stoi(argv[4]);
    splitType=argv[5][0];
    minvade=stoi(argv[6]);
    wait=stoi(argv[7]);

    cerr<<argv[0]<<" "<<filename<<" "<<kval<<" "<<nopivotPSize<<" "<<degeneracySplit<<" "<<splitType<<" "<<minvade<<endl;
    ...
    graph.kcpc_combine_temp(kval,nopivotPSize,degeneracySplit,splitType,minvade,cliques,fans);
