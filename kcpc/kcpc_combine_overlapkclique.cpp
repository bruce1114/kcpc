...
void newFinalKcpc(vector<vector<int> >& cliques,int kval,int adjlistSize,unf& fans){
    ...
    cerr<<"group num: "<<groupNum<<" focus clique num: "<<focusCliques.size()<<endl;
    cerr<<"real group num: "<<results.size()<<endl;
...
int main(int argc, char *argv[]){
    ...
    pivotIterate=stoi(argv[4]);
    wait=stoi(argv[5]);

    cerr<<argv[0]<<" "<<filename<<" "<<kval<<" "<<nopivotPSize<<" "<<pivotIterate<<"times"<<endl;
    ...
    #ifdef scalability
    filename=argv[1];
    int kval,wait,nopivotRatio,nopivotPSize,pivotIterate,savePivot,percent;
    kval=stoi(argv[2]);
    percent=stoi(argv[3]);
    wait=stoi(argv[4]);
    nopivotPSize=stoi(argv[5]);
    pivotIterate=stoi(argv[6]);

    cerr<<argv[0]<<" "<<filename<<" "<<kval<<" "<<nopivotPSize<<" "<<pivotIterate<<"times"<<" "<<percent<<endl;
