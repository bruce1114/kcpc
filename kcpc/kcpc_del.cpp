...
void updateKCPC(unf& fans,vector<vector<int> >& cliques,int da,int db,int nodeSize){
    ...
    for(int i=0;i<deletedCliques.size();++i){
        int c=deletedCliques[i];
        vector<int> subc1,subc2;
        for(int j=0;j<cliques[c].size();++j){
            int node=cliques[c][j];
            if(node!=da&&node!=db){
                subc1.push_back(node);
                subc2.push_back(node);
            }else if(node==da){
                subc1.push_back(node);
            }else{
                subc2.push_back(node);
        ...
        if(subc1.size()>=kval) candNewClique.push_back(subc1);
        if(subc2.size()>=kval) candNewClique.push_back(subc2);
    }
...
int main(int argc, char *argv[]){
    ...
    for(int i=0;i<delLines.size();++i){
        int a=delLines[i][0];
        int b=delLines[i][1];
        bool res=graph.delEdge(a,b);
        if(res==false){
            cout<<"false edge deletion"<<endl;
            continue;
        }
        delEdgeClique(a,b);
        updateKCPC(fans,cliques,a,b,graph.adjList.size());
        //report
        if(wait>0&&i%wait==0){
            gettimeofday(&end,NULL);
    ...
    cerr<<"ave time: "<<((end.tv_sec-start.tv_sec)*1000+(end.tv_usec-start.tv_usec)/1000)*1.0/delLines.size()<<endl;
